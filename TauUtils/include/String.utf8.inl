...
[[nodiscard]] static inline c32 DecodeCodePointForwardUnsafe(const c8* const str, iSys& index, const iSys inCodeUnits) noexcept
{
...
static inline iSys EncodeCodePoint(const c32 c, c8* const outString, iSys& outCodeUnit, const iSys outCodeUnits) noexcept
{
    if(c <= 0x7F) // 1 Byte
    {
...
[[nodiscard]] static inline c32 DecodeCodePointForward(const StringData<c8>& string, uSys& index) noexcept
{
...
[[nodiscard]] static inline c32 DecodeCodePointBackward(const StringData<c8>& string, uSys& index) noexcept
{
    if(index >= string.Length)
    {
        return U'\0';
...
[[nodiscard]] static inline iSys CalculateCodePoints(const c8* const inString, const iSys inCodeUnits, const iSys startCodeUnit = 0, const iSys priorCodePoints = 0) noexcept
{
...
static inline iSys Transform(const c8* const inString, c32* const outString, const iSys inCodeUnits, const iSys outCodeUnits) noexcept
{
    if(!inString || inCodeUnits <= 0)
    { return -1; }
...
static inline iSys Transform(const c32* const inString, c8* const outString, const iSys inCodeUnits, const iSys outCodeUnits) noexcept
{
    if(!inString || inCodeUnits <= 0)
    { return -1; }
...
template<>
inline DynStringCodePointIteratorT<c8>::DynStringCodePointIteratorT(const StringData& string, const uSys index) noexcept
    : m_String(string)
    , m_Index(index)
...
{
    const c8* const rawStr = string.c_str();
    ...
    if(len < 16)
    {
        ...
        if(tau::string::utf8::Transform(rawStr, newStr, static_cast<iSys>(string.length()), len))
        {
            return DynStringT<c32>();
        }
    ...
    else
    {
        ...
        if(tau::string::utf8::Transform(rawStr, newStr, static_cast<iSys>(string.length()), 16))
        {
...
inline DynStringT<c8> StringCast(const DynStringT<c32>& string) noexcept
{
    ...
    {
        void* const placement = operator new(sizeof(ReferenceCounter::Type) + (len + 1) * sizeof(c8), ::std::nothrow);
        ...
        return DynStringT<c8>::passControl(refCount, newStr, len);
    }
}
